...
class InceptionNet(nn.Module):
    def __init__(self, dropout=0.5):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=24, kernel_size=(7, 7), padding=(3, 3)),
            nn.BatchNorm2d(24),
            nn.ReLU(inplace=True),
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(in_channels=24, out_channels=12, kernel_size=(1, 1)),
            nn.BatchNorm2d(12),
            nn.Conv2d(in_channels=12, out_channels=64, kernel_size=(5, 5), padding=(2, 2)),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), ceil_mode=True),
            nn.Dropout(p=dropout),
        ...
        channel4 = [8, 16, 64, 8, 32, 24]  # 128
        self.inception3 = InceptionBlock(64, channel2[0], channel2[1], channel2[2], channel2[3], channel2[4], channel2[5])
        self.inception4 = InceptionBlock(128, channel4[0], channel4[1], channel4[2], channel4[3], channel4[4], channel4[5])
...
class Batch11(InceptionNet):
    def __init__(self, dropout=0.5):
        super(Batch11, self).__init__()

        self.vov1_1x1 = nn.Sequential(
            nn.Conv2d(kernel_size=(1, 1), in_channels=89, out_channels=64),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            # nn.Dropout(p=dropout),
        )
        self.vov2_1x1 = nn.Sequential(
            nn.Conv2d(kernel_size=(1, 1), in_channels=321, out_channels=256),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            # nn.Dropout(p=dropout),
        )
    ...
    def forward(self, inputs):  # [256, 1, 300, 13]
        x11 = self.mp4(inputs)  # [256, 1, 75, 4]
        x12 = self.conv1(inputs)  # [256, 24, 75, 4]
        x13 = self.conv2(x12)  # [256, 64, 38, 2]
        x12 = self.mp2(x12)  # [256, 24, 38, 2]
        x1 = self.vov1_1x1(torch.cat([x11, x12, x13], dim=1))

        # x21 = self.mp4(x11)  # [256, 1, 19, 1]
        x21 = self.mp2(self.mp8(inputs))
        x22 = self.mp4(x1)  # [256, 64, 19, 1]
        x23 = self.sequential(self.inception3(x1))  # [256, 128, 75, 4]
        x24 = self.mp2(x23)
        x25 = self.sequential(self.inception4(x23))

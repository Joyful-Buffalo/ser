...
class ELAN(InceptionNet):
    def __init__(self, dropout=0.5):
        super(ELAN, self).__init__()
        self.conv1x1_2 = nn.Sequential(
            nn.Conv2d(kernel_size=(1, 1), in_channels=321, out_channels=256),
            # nn.Conv2d(kernel_size=(3, 3), in_channels=193, out_channels=128, padding=(1, 1)),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            # nn.Dropout(p=dropout),
        )
    ...
    def forward(self, inputs):
        x1 = self.mp4(self.mp4(inputs))
        conv2 = self.conv2(self.conv1(inputs))
        x2 = self.mp4(conv2)
        x3 = self.sequential(self.inception4(self.sequential(self.inception3(conv2))))
        out = self.conv1x1_2(torch.cat([x1, x2, x3], dim=1))

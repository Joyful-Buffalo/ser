...
class MyDataSet(Dataset):
    ...
    def read_data(is_train=True, sr=0.7):
        ...
        target = raw_data['客户流量'].iloc[USE_DAY:].values.reshape(-1, 1)
        ...
        return data[:int(len(raw_data) * sr)] if is_train else data[
...
def to_device(i, j):
    ...
    j = j.to(device).float().transpose(0, 1)
    j = j.reshape(-1, OUTPUT_SIZE)
    return i, j
...
if __name__ == '__main__':
    ...
    # 优化器,学习率为0.01
    optimizer = torch.optim.Adam(lstm_model.parameters(), lr=10 ** -6)
    main(EPOCHS)

...
# time_emb中使用 GELU, resnet中使用SiLu
...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, t_list):
        device = t_list.device
        half_dim = self.dim // 2
        emb = math.log(10000) / (half_dim - 1)
        emb = torch.exp(torch.arange(half_dim, device=device) * -emb)
        emb = t_list[:, None] * emb[None, :]
...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups=4):
        ...
        self.block = nn.Sequential(
            ...
            nn.GroupNorm(groups, dim_out),  # groups=4, dim_out=28
            nn.SiLU()
        )
...
class Unet(nn.Module):
    def __init__(
        ...
        init_dim = default(init_dim, dim // 3 * 2)  # 18
        ...
        self.final_conv = nn.Sequential(
            block_klass(dim, dim),
            nn.Conv2d(dim, self.out_dim, kernel_size=(1, 1))
        )

    def forward(self, sample_list, time_list):
        x = self.init_conv(sample_list)  # [128, 18, 28, 28]
        t = self.time_mlp(time_list) if exists(self.time_mlp) else None  # [128, 112]
        h = []
        # downsample
...
def extract(a, t_list, x_shape):
    b, *_ = t_list.shape
    out = a.gather(-1, t_list)
...
class Trainer(object):
    ...
    def save(self, milestone):
        data = {
            'step': self.step,
            'batch_model': self.model.state_dict(),
            'ema': self.ema_model.state_dict(),
            'scaler': self.scaler.state_dict()
        }
        torch.save(data, str(self.results_folder / f'batch_model-{milestone}.pt'))

    def load(self, milestone):
        data = torch.load(str(self.results_folder / f'batch_model-{milestone}.pt'))

        self.step = data['step']
        self.model.load_state_dict(data['batch_model'])

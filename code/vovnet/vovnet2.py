...
class VovNet2(InceptionNet):
    def __init__(self, dropout=0.5):
        super(VovNet2, self).__init__()
        self.conv7x7 = nn.Conv2d(in_channels=1, out_channels=24, kernel_size=(7, 7), padding=(3, 3))
        self.sequential7x7to5x5 = nn.Sequential(
            nn.BatchNorm2d(24),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), ceil_mode=True),
            nn.Dropout(p=dropout)
        )
        self.sequential7x7to1x1 = nn.Sequential(
            nn.BatchNorm2d(24),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=(4, 4), stride=(4, 4), ceil_mode=True),
            nn.Dropout(p=dropout),
        )
        self.conv5x5 = self.conv2

        self.conv4x4_1 = nn.Sequential(
            # nn.Conv2d(in_channels=1, out_channels=8, kernel_size=(2, 2), stride=(2, 2), padding=(0, 1)),
            # nn.BatchNorm2d(8),
            # nn.ReLU(inplace=True),
            # # nn.MaxPool2d(kernel_size=(4, 4), stride=(4, 4), ceil_mode=True),
            # nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), ceil_mode=True),
        ...
        self.conv4x4_2 = nn.Sequential(
            # nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(2, 2), stride=(2, 2), padding=(1, 0)),
            # # nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), ceil_mode=True),
            # nn.BatchNorm2d(64),
            # nn.ReLU(inplace=True),
            # nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), ceil_mode=True),
        ...
        self.conv16x16 = nn.Sequential(
            # nn.Conv2d(in_channels=1, out_channels=8, kernel_size=(4, 4), stride=(4, 4), padding=(2, 2)),
            # nn.BatchNorm2d(8),
            # nn.ReLU(inplace=True),
            # nn.MaxPool2d(kernel_size=(4, 4), stride=(4, 4), ceil_mode=True),
            # nn.Dropout(p=dropout),
        ...
        self.conv1x1 = nn.Sequential(
            nn.Conv2d(in_channels=89, out_channels=64, kernel_size=(1, 1))
        )
        self.inception3ToInception4 = nn.Sequential(
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True),
            # nn.Dropout(p=dropout),
        )
        self.inception3To1x1 = nn.Sequential(
            # nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(2, 2), stride=(2, 2), padding=(1, 0)),
            # # nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), ceil_mode=True),
            # nn.BatchNorm2d(128),
            # nn.ReLU(inplace=True),
            # nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), ceil_mode=True),
        ...
        self.inception4To1x1 = nn.Sequential(
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), ceil_mode=True),
            # nn.Dropout(p=dropout),
        )
        self.conv1x1_2 = nn.Sequential(
            nn.Conv2d(kernel_size=(1, 1), in_channels=321, out_channels=128),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            # nn.Dropout(p=dropout)
        )
    ...
    def forward(self, inputs):
        x11 = self.conv4x4_1(inputs)  # [256, 1, 75, 4]
        conv7x7 = self.conv7x7(inputs)  # [256, 24, 300, 13]
        x12 = self.sequential7x7to1x1(conv7x7)  # [256, 24, 75, 4]
        x13 = self.conv5x5(self.sequential7x7to5x5(conv7x7))  # [256, 64, 75, 4]
        x1 = self.conv1x1(torch.cat([x11, x12, x13], dim=1))  # [256, 64, 75, 4]

        x21 = self.conv16x16(inputs)  # [256, 1, 19, 1]
        x22 = self.conv4x4_2(x1)  # [256, 64, 19, 1]
        inception3 = self.inception3(x1)  # [256, 128, 75, 4]
        x23 = self.inception3To1x1(inception3)  # [256, 128, 19, 1]
        x24 = self.inception4To1x1(self.inception4(self.inception3ToInception4(inception3)))
        x2 = self.conv1x1_2(torch.cat([x21, x22, x23, x24], dim=1))

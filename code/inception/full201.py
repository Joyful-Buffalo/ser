...
class InceptionNet1(nn.Module):
    def __init__(self, dropout=0.5):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(in_channels=96, out_channels=32, kernel_size=(1, 1)),
            nn.BatchNorm2d(32),
        )
        #        c1, c5_in, c5_out c3_in, c3_out, cb,
        channel1 = [4, 8, 32, 4, 16, 12]  # 64
        channel2 = [8, 16, 64, 8, 32, 24]  # 128
        channel4 = [16, 32, 128, 16, 64, 48]  # 256
        # channel4 = [8, 16, 64, 8, 32, 24]
        self.inception2 = InceptionBlock(32, channel1[0], channel1[1], channel1[2], channel1[3], channel1[4],
                                         channel1[5])
        ...
        self.sequential0 = nn.Sequential(
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True),
            nn.Dropout(p=0.3)
        )
    ...
    def forward(self, x):
        ...
        x = self.sequential0(self.inception2(x))

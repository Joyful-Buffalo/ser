...
class Head(nn.Module):
    def __init__(self, dropout=0.5, in_channel=1, out_channel=32):
        super().__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=(3, 3), padding=1),
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=(9, 1), padding=(4, 0)),
            nn.BatchNorm2d(out_channel),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True),
            nn.Dropout(p=dropout),
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=(1, 11), padding=(0, 5)),
            nn.BatchNorm2d(out_channel),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True),
            nn.Dropout(p=dropout),
...
class InceptionNet1(nn.Module):
    def __init__(self, dropout=0.5):
        ...
        self.conv1 = Head(dropout=0.5)
        self.head2 = Head(dropout=0.3, in_channel=16, out_channel=48)
        self.conv2 = nn.Sequential(
        ...
        self.inception3 = InceptionBlock(48 * 3, channel2[0], channel2[1], channel2[2], channel2[3], channel2[4],
                                         channel2[5])
...
# full16+采用两个新head  # input187 frame , MFLOPs = 150+M
class FullInception11(nn.Module):
    def __init__(self, frame, column, output_size, dropout=0.5):
        super(FullInception11, self).__init__()
        self.frame = frame
        self.column = column

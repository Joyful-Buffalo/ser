...
#  传统末尾dnn
class FullInception4(nn.Module):
    def __init__(self, frame, column, output_size, dropout=0.5):
        super(FullInception4, self).__init__()
        self.frame = frame
        self.column = column
        ...
        for i in range(6):
            if in_feature // 2 < in_feature / 2:
                ...
                in_feature /= 2
        self.in_feature = int(in_feature * output_size)
        self.fc = nn.Linear(in_features=self.in_feature, out_features=output_size)
    ...
    def forward(self, x):
        ...
        conv_out = conv_out.view(-1, self.in_feature)
